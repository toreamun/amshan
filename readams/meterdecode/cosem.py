...
PhysicalUnits = construct.Enum(construct.Int8ub, W=27, var=29, Wh=30, varh=32, A=33, V=35)
...
DateTime = construct.Struct(
    ...
    "deviation" / construct.ExprAdapter(
        construct.Int16sb,
    ...
    "datetime" / construct.Computed(lambda ctx: datetime.datetime(
...
ObisCodeOctedStringField = construct.FocusedSeq(
    "code",
    construct.Const(CommonDataTypes.octet_string, CommonDataTypes),  # expect octet string (bytes)
...
VisibleStringField = construct.FocusedSeq(
    "value",
    construct.Const(CommonDataTypes.visible_string, CommonDataTypes),  # An ordered sequence of ASCII characters
    "value" / VisibleString
)
...
Scaler = construct.Struct(
    "exponent" / IntegerField,  # This is the exponent (to the base of 10) of the multiplication factor.
...
def scaled_simple_value(unit, scale_exponent):
    return construct.Struct(
        ...
        "scaled_value" / construct.Computed(construct.this.unscaled_value * construct.this.scale),
...
OptionalDateTimeField = construct.FocusedSeq(
    "value",
    "content_type" / CommonDataTypes,
    construct.Check(lambda ctx:
            ctx.content_type == CommonDataTypes.null_data
            or ctx.content_type == CommonDataTypes.octet_string),
    "value" / construct.If(
        construct.this.content_type != CommonDataTypes.null_data,
        DateTime
    )
)
...
LongInvokeIdAndPriority = construct.BitStruct(
    ...
    construct.Padding(4),
    "self-descriptive" / construct.Enum(construct.BitsInteger(1), NotSelfDescriptive=0, SelfDescriptive=1),
    "processing-option" / construct.Enum(construct.BitsInteger(1), ContinueOnError=0, BreakOnError=1),
    "service-class" / construct.Enum(construct.BitsInteger(1), Unconfirmed=0, Confirmed=1),
